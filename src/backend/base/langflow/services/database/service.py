...
from langflow.utils.migration import get_migration_lock_key, to_sync_database_url
...
class DatabaseService(Service):
    ...
    def _run_migrations(self, fix) -> None:
        ...
        with buffer_context as buffer:
            ...
            alembic_cfg.attributes["migration_lock_held"] = False

            from sqlalchemy import create_engine

            sync_engine = create_engine(to_sync_database_url(self.database_url))
            try:
                with sync_engine.begin() as connection:
                    alembic_cfg.attributes["connection"] = connection
                    if connection.dialect.name == "postgresql":
                        lock_key = get_migration_lock_key()
                        alembic_cfg.attributes["migration_lock_held"] = True
                        connection.execute(text("SET LOCAL lock_timeout = '180s';"))
                        connection.execute(text(f"SELECT pg_advisory_xact_lock({lock_key});"))

                    should_initialize_alembic = not inspect(connection).has_table("alembic_version")
