...
async def download_media(
    ...
    """Download media file (image, video, audio) from storage for browser rendering."""
    storage_service = get_storage_service()
    extension = file_name.split(".")[-1]
    flow_id_str = str(flow_id)
    ...
    if not content_type.startswith(("image", "video", "audio")):
        raise HTTPException(status_code=500, detail=f"Content type {content_type} is not a supported media type")
