...
test.describe("ModelProviderCount Component", () => {
  test("should open model provider page when navigating via settings", {
  ...
  test("should display provider list in the page", {
    tag: ["@release", "@components", "@workspace"],
  }, async ({ page }) => {
    await awaitBootstrapTest(page);
  ...
  test("model provider page should display provider count information", {
    tag: ["@release", "@components", "@workspace"],
  }, async ({ page }) => {
    await awaitBootstrapTest(page);
