...
test("vector store from starter projects should have its connections and nodes on the flow", {
  ...
  const astraStarterProject = responseBody.find((project: any) => {
    if (project.data.nodes) {
      return project.data.nodes.some((node: any) => node.id.includes("Astra"));
    }
  });
