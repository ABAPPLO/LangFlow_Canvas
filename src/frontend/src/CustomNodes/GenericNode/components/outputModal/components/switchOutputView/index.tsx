...
import MediaOutputView from "./components/mediaOutputView";
import { extractMediaUrls } from "./components/mediaOutputView/utils";
...
const SwitchOutputView: React.FC<SwitchOutputViewProps> = ({
  ...
  const mediaUrls = useMemo(
    () => extractMediaUrls(resultMessageMemoized),
  ...
  return type === "Outputs" ? (
    <>
      ...
      <Case condition={mediaUrls.length > 0 && !showRaw && !isToolOutput}>
        <MediaOutputView
          urls={mediaUrls}
        ...
        <div className="relative flex min-h-0 flex-1 flex-col">
          {mediaUrls.length > 0 && showRaw && (
            <button
              ...
              className="absolute right-0 top-0 z-10 flex items-center gap-1 rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-muted hover:text-foreground"
            >
