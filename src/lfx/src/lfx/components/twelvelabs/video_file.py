...
class VideoFileComponent(BaseFileComponent):
    ...
    def load_files(self) -> DataFrame:
        ...
        if not hasattr(self, "file_path") or not self.file_path:
            msg = "No video file path provided"
            raise ValueError(msg)
        ...
        if not file_path_obj.exists():
            msg = f"Video file not found at path: {self.file_path}"
        ...
        self.status = f"Loaded video file: {self.file_path}"
        return DataFrame(data=[video_data])
